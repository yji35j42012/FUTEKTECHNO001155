$(document).ready(function () {
	...
	function scrollNav(dom) {
		if (dom[1].offsetTop <= $(document).scrollTop()) {
			$('.mod-tab .line').stop().animate({
				'left': $("#m_com").position().left + 'px',
				'width': $("#m_com").width()
			}, 200);
			$("#m_abo").removeClass('active-tab');
			$("#m_com").addClass('active-tab');
		} else {
			$('.mod-tab .line').stop().animate({
				'left': $("#m_abo").position().left + 'px',
				'width': $("#m_abo").width()
			}, 200);
			$("#m_com").removeClass('active-tab');
			$("#m_abo").addClass('active-tab');
		}
	}
});