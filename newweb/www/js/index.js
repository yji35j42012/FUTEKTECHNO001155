...
function setupMenu() {
		...
		for (var i = 0; i < langx_second_menu.children.length; i++){
			...
			langx_second_menu.onchange = function(e){
				...
				if(window.location.href.indexOf("ti666")!=-1){
					window.location.href = location.protocol+"//ti666.cvssp2017.com?langx="+langx
				}else if(window.location.href.indexOf("icash99")!=-1){
					window.location.href = location.protocol+"//"+window.location.host+"?langx="+langx
				}else {
					window.location.href = location.protocol+"//"+window.location.host+"?langx="+langx
				}
			};
	...
	document.getElementById("menu_btn_hm8").onclick = function () {
		menu_btn.classList.remove("active");
		scrollTo(document.getElementById("hm_7").offsetTop);
	};
